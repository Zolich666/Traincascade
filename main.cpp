...
int main()
{
	...
	cascade.load("F:\\Adaboost\\0214MLTest\\cascade.xml");
	IplImage* pImg;
	int Scores[20];
	ofstream out_txt("d.txt");
	out_txt.close();
	for (k = 1; k < 42; k++)
	{
		char indir[50],outdir[50];
		sprintf(indir, "F:\\Adaboost\\0214MLTest\\test\\test_%03d.jpg", k);
		sprintf(outdir, "test/test_%03d", k);
		TestImage = imread(indir);
		CopyTest = TestImage.clone();

		ofstream out_txt("d.txt", ios::app);
		...
		cout << outdir << endl << goals_final.size() << endl;
		out_txt << outdir << endl << goals_final.size() << endl;
		out_txt.close();
		...
		imshow(outdir, CopyTest);
		waitKey(1000);
		destroyWindow(outdir);
	}
